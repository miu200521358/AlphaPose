...
def pose_nms_fullbody(bboxes, bbox_scores, bbox_ids, pose_preds, pose_scores, areaThres=0):
    ...
    while mask.any():
        ...
        num_match_keypoints = PCK_match_fullbody(
            pose_preds[:, :, :][tensor_mask][pick_id], pose_scores[:, :, :][tensor_mask][pick_id], pose_preds[:, :, :][tensor_mask], ref_dist
        )
...
def p_merge(ref_pose, cluster_preds, cluster_scores, ref_dist):
    ...
    """
    dist = torch.sqrt(torch.sum(torch.pow(ref_pose[np.newaxis, :] - cluster_preds, 2), dim=2))  # [n, kp_num]
    ...
    for i in range(kp_num):
        ...
        cluster_joint_location = cluster_preds[:, i, :][mask[:, i].unsqueeze(-1).repeat(1, 2)].view((torch.sum(mask[:, i]), -1))
...
def get_parametric_distance(i, all_preds, keypoint_scores, ref_dist, use_dist_mask=False):
    ...
    if use_dist_mask:
        ...
        final_dist = (
            torch.mean(score_dists[:, :-110], dim=1)
            + torch.mean(score_dists[:, -110:-42], dim=1) * face_weight_score
            + torch.mean(score_dists[:, -42:], dim=1) * hand_weight_score
            + mu
            * (
                torch.mean(point_dist[:, :-110], dim=1)
                + torch.mean(point_dist[:, -110:-42], dim=1) * face_weight_dist
                + torch.mean(point_dist[:, -42:], dim=1) * hand_weight_dist
            )
        )
    else:
...
def PCK_match_fullbody(pick_pred, pred_score, all_preds, ref_dist):
    ...
    ref_dist = min(ref_dist, 7)
    num_match_keypoints_body = torch.sum(dist[:, :26] / ref_dist <= 1, dim=1)

    num_match_keypoints_face = torch.sum(dist[:, 26:94] / ref_dist <= face_factor, dim=1)

    num_match_keypoints_hand = torch.sum(dist[:, 94:] / ref_dist <= hand_factor, dim=1)
